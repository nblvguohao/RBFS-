...
def generate_excel(expenses):
    workbook = xlsxwriter.Workbook('C:\\Users\\123\\Desktop\\夏生荣论文\\夏生荣论文实验\\data\\dataset\\Location_List_MBD.xlsx')
...
def result_excel(expenses):
    workbook = xlsxwriter.Workbook('C:\\Users\\123\\Desktop\\lvguohao\\\data\\dataset\\PPIWeightResult_MBD.xlsx')
    worksheet = workbook.add_worksheet()
    # 设定格式，等号左边格式名称自定义，字典中格式为指定选项
    # bold：加粗，num_format:数字格式
    bold_format = workbook.add_format({'bold': True})
...
if __name__ == '__main__':
    ...
    dataPath = os.path.join(parent_path, r'data\dataset\result_List_MBD.xlsx')
    list = pd.read_excel(dataPath)
    ...
    workbook = xlsxwriter.Workbook('C:\\Users\\123\\Desktop\\lvguohao\\data\\dataset\\eachProteinDomainWeight_MBD.xlsx')
    worksheet = workbook.add_worksheet()
    # 设定格式，等号左边格式名称自定义，字典中格式为指定选项
    # bold：加粗，num_format:数字格式
    bold_format = workbook.add_format({'bold': True})
    ...
    workbook = xlsxwriter.Workbook('C:\\Users\\123\\Desktop\\lvguohao\\data\\dataset\\proteinDomainWeight_MBD.xlsx')
    worksheet = workbook.add_worksheet()
    # 设定格式，等号左边格式名称自定义，字典中格式为指定选项
    # bold：加粗，num_format:数字格式
    bold_format = workbook.add_format({'bold': True})
    ...
    workbook = xlsxwriter.Workbook('C:\\Users\\123\\Desktop\\lvguohao\\data\\dataset\\proteinSubcellularLocationWeight_MBD.xlsx')
    worksheet = workbook.add_worksheet()
    bold_format = workbook.add_format({'bold': True})
    # 将二行二列设置宽度为15(从0开始)
    worksheet.set_column(1, 1, 15)
